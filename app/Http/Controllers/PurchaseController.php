...
class PurchaseController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($role->hasPermissionTo('purchases-index')) {
            ...
            $purchases   =   Purchase::latest()->with('supplier')->get();
        ...
        else
        {
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
                $purchases =  Purchase::select('purchases.*')
                            ...
                            ->orwhere([
                                ['purchases.supplier_id', 'LIKE', "%{$search}%"],
                ...
                $purchases =  Purchase::select('purchases.*')
                            ...
                            ->leftJoin('suppliers', 'purchases.supplier_id', '=', 'suppliers.id')
                            ->whereDate('purchases.created_at', '=' , date('Y-m-d', strtotime(str_replace('/', '-', $search))))
                            ->orwhere('purchases.supplier_id', 'LIKE', "%{$search}%")
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('purchases-add')){
            ...
            return view('purchase.create', compact('lims_supplier_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        $supplier_data  =   $supplier->data ?   json_decode($supplier->data, true)    :   null;
        $products      =   [];
        $total_qty     =   0;
        $total_Amount  =   0;
        ...
        foreach ($supplier_data as $key => $value)
        {
            ...
            if ($request->$amount)
            {
                $supplier_data[$key]["amount"] = $request->$amount;
            }
        }
        ...
        foreach($supplier_data   as $key =>  $value)
        {
            ...
            $products      +=   [
                $key    =>  [
                    'status'    =>  $supplier_data[$key]["status"]   ?   1   :   0,
                    'time'      =>  [
                        'm_status'  =>  $supplier_data[$key]["time"]["m_status"]   ?   1   :   0,
                        'e_status'  =>  $supplier_data[$key]["time"]["e_status"]   ?   1   :   0,
                    ],
                    'qty'       =>  $key == 'wanda'
                                    ?   $request->qty
                                    :   [
            ...
            $total_qty      +=  $key == 'wanda' ?   $request->qty   :   $request->$m_qty + $request->$e_qty;
        ...
        $supplier->update([
            'data'  =>  json_encode($supplier_data),
        ]);
    ...
    public function createPurchase($id)
    {
        ...
        if($role->hasPermissionTo('purchases-add'))
        {
            ...
            if($supplier)
            {
                ...
                return view('purchase.purchase-create', compact('lims_supplier_list', 'lims_warehouse_list', 'lims_tax_list', 'lims_product_list_without_variant', 'lims_product_list_with_variant','supplier'));
            }
            else
            {
                return redirect()->back()->with('not_permitted', 'Supplier not found');
            }
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    ...
    public function edit($id)
    {
        ...
        if($role->hasPermissionTo('purchases-edit')){
            ...
            $lims_product_list_without_variant = $this->productWithoutVariant();
            $lims_product_list_with_variant = $this->productWithVariant();
            $lims_purchase_data = Purchase::with('supplier')->find($id);
            $lims_product_purchase_data = ProductPurchase::where('purchase_id', $id)->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        if($purchase_data)
        {
            ...
            {
                $message = 'Something went wrong while updating purchase';
            }
        }

        return redirect('purchases')->with('message', $message);
    }
